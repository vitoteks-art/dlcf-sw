...
function PublicationPreview({ form }) {
  return (
    <article className="wp-publication-preview">
      {form.cover_image_url ? <figure><img src={assetUrl({ url: form.cover_image_url })} alt={form.cover_alt || form.title} />{form.cover_caption ? <figcaption>{form.cover_caption}</figcaption> : null}</figure> : null}
...
export default function AdminPublications({ user, states = [], setStatus, canManagePublications, canPublishMedia, uploadImage }) {
  ...
  const handleFileUpload = async (file, field) => {
    ...
    try {
      ...
      const rawUrl = await uploadImage(file, { scope: current.scope, state: current.state, usage_context: field === "file_url" ? "publication" : "publication", title: file.name });
      const url = assetUrl({ url: rawUrl });
  ...
  const selectMedia = (asset) => {
    const normalized = { ...asset, url: assetUrl(asset) };
    if (mediaModal.onSelect) mediaModal.onSelect(normalized);
    else if (mediaModal.mode === "cover") setForm((prev) => ({ ...prev, cover_image_url: normalized.url, cover_alt: prev.cover_alt || normalized.title }));
    else if (mediaModal.mode === "file") setForm((prev) => ({ ...prev, file_url: normalized.url }));
    closeMediaLibrary();
  };
      ...
      {editorOpen && canManagePublications ? <div className="wp-editor-shell card">
        ...
        <div className="wp-editor-layout">
          <main className="wp-editor-main">
            ...
            {form.file_url ? <div className="upload-preview"><strong>Attached publication file</strong><p><a href={assetUrl({ url: form.file_url })} target="_blank" rel="noreferrer">{assetNameFromUrl(form.file_url)}</a></p></div> : null}
          ...
          <aside className="wp-editor-sidebar">
            ...
            <div className="card soft-card"><h4>Featured Image</h4>{form.cover_image_url ? <figure className="featured-image-preview"><img src={assetUrl({ url: form.cover_image_url })} alt={form.cover_alt || form.title} />{form.cover_caption ? <figcaption>{form.cover_caption}</figcaption> : null}</figure> : <div className="featured-placeholder">No featured image selected</div>}<div className="form-actions stacked"><p className="upload-limit-note">Images up to 25MB.</p><label className="button-like-upload">Upload Featured Image<input type="file" accept="image/*" onChange={(e) => handleFileUpload(e.target.files?.[0], "cover_image_url")} /></label><button type="button" className="btn-sm btn-outline" onClick={() => openMediaLibrary("cover")}>Choose from Library</button>{form.cover_image_url ? <button type="button" className="btn-sm btn-danger" onClick={() => setForm({ ...form, cover_image_url: "" })}>Remove</button> : null}</div><label>Alt Text<input value={form.cover_alt || ""} onChange={(e) => setForm({ ...form, cover_alt: e.target.value })} /></label><label>Caption<input value={form.cover_caption || ""} onChange={(e) => setForm({ ...form, cover_caption: e.target.value })} /></label></div>
