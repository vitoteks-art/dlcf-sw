  ...
  return (
    <div className="media-modal-backdrop" role="dialog" aria-modal="true">
      <div className="media-modal card">
        <div className="section-header media-modal-header"><div><h3>Media Library</h3><p className="lede">Upload or reuse images and files for this publication.</p></div><button type="button" className="btn-sm btn-outline" onClick={onClose}>Close</button></div>
        ...
        {tab === "upload" ? <div className="wp-upload-drop"><strong>Drop or choose a file</strong><p>Images, PDF documents, audio, and video are supported. Images can be inserted directly into content.</p><input type="file" accept="image/*,application/pdf,.doc,.docx,audio/*,video/*" disabled={uploading} onChange={(e) => handleUpload(e.target.files?.[0])} />{uploading ? <p className="text-success">Uploading…</p> : null}</div> : <>
      ...
      {editorOpen && canManagePublications ? <div className="wp-editor-shell card">
        ...
        <div className="wp-editor-layout">
          ...
          <aside className="wp-editor-sidebar">
            ...
            <div className="card soft-card"><h4>Featured Image</h4>{form.cover_image_url ? <figure className="featured-image-preview"><img src={form.cover_image_url} alt={form.cover_alt || form.title} />{form.cover_caption ? <figcaption>{form.cover_caption}</figcaption> : null}</figure> : <div className="featured-placeholder">No featured image selected</div>}<div className="form-actions stacked"><label className="button-like-upload">Upload Featured Image<input type="file" accept="image/*" onChange={(e) => handleFileUpload(e.target.files?.[0], "cover_image_url")} /></label><button type="button" className="btn-sm btn-outline" onClick={() => openMediaLibrary("cover")}>Choose from Library</button>{form.cover_image_url ? <button type="button" className="btn-sm btn-danger" onClick={() => setForm({ ...form, cover_image_url: "" })}>Remove</button> : null}</div><label>Alt Text<input value={form.cover_alt || ""} onChange={(e) => setForm({ ...form, cover_alt: e.target.value })} /></label><label>Caption<input value={form.cover_caption || ""} onChange={(e) => setForm({ ...form, cover_caption: e.target.value })} /></label></div>
